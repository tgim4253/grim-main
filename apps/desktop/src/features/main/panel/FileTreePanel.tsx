...
import { useHoverOpen, useMultiSelect, parseDropTarget, getNodeDragPayload } from '@tgim/dnd/index';
...
export const FileTree = () => {
  ...
  useDndMonitor({
    onDragStart: event => {
      ...
      if (!payload) return;
      setActiveId(depthMap.has(payload.nodeId) ? payload.nodeId : null);
      if (payload.source === 'file-tree') {
        onDragStartSelect(payload.nodeId);
        updateSelectedNode(payload.nodeId);
      }
    },
    onDragCancel: () => {
      setActiveId(null);
    ...
    onDragOver: event => {
      ...
      if (!payload) return;
      const selection =
        payload.selection && payload.selection.length > 0 ? payload.selection : [payload.nodeId];
      onDragOverHoverOpen(event.over?.id, selection);
    },
    onDragEnd: event => {
      setActiveId(null);
      const payload = getNodeDragPayload(event.active.data.current);
      ...
      const selection =
        payload.selection && payload.selection.length > 0 ? payload.selection : [payload.nodeId];
      const validSelection = selection.filter(id => depthMap.has(id));
      ...
      onMove({
        dragIds: validSelection,
        parentId: target.id,
        index: 0,
      });

      setExpanded(prev => (target.id !== 'root' ? new Set(prev).add(target.id) : prev));
      resetHoverOpen();
    },
  });
