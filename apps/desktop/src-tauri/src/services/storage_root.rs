...
mod platform_impl {
    ...
    pub fn find_mount_point(p: &Path) -> Option<PathBuf> {
        ...
        if let Ok(mps) = crate::services::storage_root::get_mount_paths() {
            for mp in mps {
                ...
                {
                    match &best {
                        Some(cur) if cur.as_os_str().len() >= mp.as_os_str().len() => {}
    ...
    pub fn find_device_and_fstype(mount_path: &str) -> (Option<String>, Option<String>) {
        // Example lines:
    ...
    pub fn query_disk_ids(device: Option<&str>) -> (String, String, String) {
        if let Some(dev) = device {
            ...
            if let Ok(out) =
                std::process::Command::new("diskutil").args(["info", "-plist", dev_arg]).output()
            {
                if out.status.success() {
                    if let Ok(value) = plist::Value::from_reader_xml(out.stdout.as_slice()) {
                        ...
                        let label = get_s("VolumeName")
                            .unwrap_or_else(|| format!("Unknown - {}", secondary));
...
mod platform_impl {
    ...
    use windows::Win32::Storage::FileSystem::{
        ...
        GetVolumePathNameW, QueryDosDeviceW,
    };
    ...
    pub fn find_mount_point(p: &Path) -> Option<PathBuf> {
        ...
        let ok = unsafe { GetVolumePathNameW(PCWSTR(w_path.as_ptr()), &mut out).is_ok() };
        if ok {
    ...
    fn resolve_root(input: &str) -> Option<String> {
        ...
        let ok = unsafe { GetVolumePathNameW(PCWSTR(in_w.as_ptr()), &mut root_w).is_ok() };
        if ok {
        ...
        if input.ends_with('\\') && (input.contains(':') || input.starts_with(r"\\")) {
    ...
    pub fn find_device_and_fstype(mount_path: &str) -> (Option<String>, Option<String>) {
        ...
        let ok = unsafe { GetVolumePathNameW(PCWSTR(path_w.as_ptr()), &mut root_buf).is_ok() };
        if !ok {
            return (None, None);
        }
        ...
        let ok_guid =
            unsafe { GetVolumeNameForVolumeMountPointW(PCWSTR(root_w.as_ptr()), &mut guid_buf) }
                .is_ok();
        ...
        let fstype = if ok_fs { utf16_buf_to_string(&fs_name_buf) } else { None };
    ...
    pub fn query_disk_ids(device: Option<&str>) -> (String, String, String) {
        if let Some(dev) = device {
            ...
            let ok_guid =
                unsafe { GetVolumeNameForVolumeMountPointW(PCWSTR(root_w.as_ptr()), &mut guid_w) }
                    .is_ok();
                ...
                && (root.as_bytes()[2] == b'\\' || root.as_bytes()[2] == b'/')
            {
                ...
                let n = unsafe { QueryDosDeviceW(PCWSTR(dos_w.as_ptr()), Some(&mut map_w)) };
                if n != 0 {
            ...
            let fstype = if ok_info { utf16_buf_to_string(&fsname_w) } else { None };

            // todo: label always return error. windows
            let label = if ok_info {
                utf16_buf_to_string(&volname_w)
                    .unwrap_or_else(|| format!("Unknown-{}", fstype.unwrap_or("".into())))
            } else {
    ...
    pub fn classify_kind(
        ...
        if let Some(ref fs) = fs_lc {
            if matches!(fs.as_str(), "cifs" | "nfs" | "smb" | "webdav" | "sshfs" | "fuse") {
                return StorageKind::Network;
            }
...
use platform_impl::{classify_kind, find_device_and_fstype, find_mount_point, query_disk_ids};

pub fn get_mount_paths() -> Result<Vec<PathBuf>> {
    // Thin wrapper to allow unified error type (anyhow::Result)
